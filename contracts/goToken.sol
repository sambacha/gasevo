...
/**
 * @title GoToken Contract
 * @notice base for any token 
 * @author Based off of Compound
 */
contract goToken is EIP20Interface, Exponential, TokenErrorReporter, ReentrancyGuard {
    /**
     * @notice Indicator that this is a GoToken contract (for inspection)
     */
    bool public constant isGoToken = true;
    ...
    /**
     * @notice Contract which oversees inter-GoToken operations
     */
    ...
    /**
     * @notice Initial exchange rate used when minting the first GoTokens (used when totalSupply = 0)
     */
     ...
     */
    event LiquidateBorrow(address liquidator, address borrower, uint repayAmount, address GoTokenCollateral, uint seizeTokens);
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        (mathErr, srGoTokensNew) = subUInt(accountTokens[src], tokens);
        if (mathErr != MathError.NO_ERROR) {
        ...
        accountTokens[src] = srGoTokensNew;
    ...
    function getAccountSnapshot(address account) external view returns (uint, uint, uint, uint) {
        uint GoTokenBalance = accountTokens[account];
        uint borrowBalance;
        ...
        return (uint(Error.NO_ERROR), GoTokenBalance, borrowBalance, exchangeRateMantissa);
    }
    ...
    /**
     * @notice Returns the current per-block borrow interest rate for this GoToken
    ...
    /**
     * @notice Returns the current per-block supply interest rate for this GoToken
    ...
    /**
     * @notice Calculates the exchange rate from the underlying to the GoToken
    ...
    /**
     * @notice Get cash balance of this GoToken in the underlying asset
    ...
    /**
     * @notice Sender supplies assets into the market and receives GoTokens in exchange
    ...
    /**
     * @notice User supplies assets into the market and receives GoTokens in exchange
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint) {
        ...
        /*
         * We get the current exchange rate and calculate the number of GoTokens to be minted:
        ...
        /*
         * We calculate the new total supply of GoTokens and minter token balance, checking for overflow:
         ...
         *  Note: The GoToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the GoToken holds an additional mintAmount of cash.
    ...
    /**
     * @notice Sender redeems GoTokens in exchange for the underlying asset
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param redeemTokens The number of GoTokens to redeem into underlying
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    ...
    /**
     * @notice Sender redeems GoTokens in exchange for a specified amount of underlying asset
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
    ...
    /**
     * @notice User redeems GoTokens in exchange for the underlying asset
     * @dev Assumes interest has already been accrued up to the current block
     * @param redeemer The address of the account which is redeeming the tokens
     * @param redeemTokensIn The number of GoTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be zero)
     * @param redeemAmountIn The number of GoTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be zero)
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
         ...
         *  Note: The GoToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the GoToken has redeemAmount less of cash.
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
         ...
         *  Note: The GoToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the GoToken borrowAmount less of cash.
         *  If doTransferOut fails despite the fact we checked pre-conditions,
         *   we revert because we can't be sure if side effects occurred.
         */
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint) {
         ...
         *  Note: The GoToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the GoToken holds an additional repayAmount of cash.
         *  If doTransferIn fails despite the fact we checked pre-conditions,
         *   we revert because we can't be sure if side effects occurred.
         */
     ...
     * @param borrower The borrower of this GoToken to be liquidated
     * @param GoTokenCollateral The market in which to seize collateral from the borrower
     * @param repayAmount The amount of the underlying borrowed asset to repay
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, GoToken GoTokenCollateral) internal nonReentrant returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
        ...
        error = GoTokenCollateral.accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted liquidation failed
        ...
        return liquidateBorrowFresh(msg.sender, borrower, repayAmount, GoTokenCollateral);
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, GoToken GoTokenCollateral) internal returns (uint) {
        /* Fail if liquidate not allowed */
        uint allowed = comptroller.liquidateBorrowAllowed(address(this), address(GoTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
        ...
        /* Verify GoTokenCollateral market's block number equals current block number */
        if (GoTokenCollateral.accrualBlockNumber() != getBlockNumber()) {
        ...
        (uint amountSeizeError, uint seizeTokens) = comptroller.liquidateCalculateSeizeTokens(address(this), address(GoTokenCollateral), repayAmount);
        ...
        if (seizeTokens > GoTokenCollateral.balanceOf(borrower)) {
        ...
        uint seizeError = GoTokenCollateral.seize(liquidator, borrower, seizeTokens);
        require(seizeError == uint(Error.NO_ERROR), "token seizure failed");

        /* We emit a LiquidateBorrow event */
        emit LiquidateBorrow(liquidator, borrower, repayAmount, address(GoTokenCollateral), seizeTokens);

        /* We call the defense hook */
        comptroller.liquidateBorrowVerify(address(this), address(GoTokenCollateral), liquidator, borrower, repayAmount, seizeTokens);
     ...
     * @dev Will fail unless called by another GoToken during the process of liquidation.
     *  Its absolutely critical to use msg.sender as the borrowed GoToken and not a parameter.
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of GoTokens to seize
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
